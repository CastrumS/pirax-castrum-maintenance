...
- Real scoped R2 verifies warning/failure publication and status, forms-only no-baseline access, manifest-last/no trace uploads, private HTML rendering, and cleanup. `report.test.ts` separately proves newer forms manifests do not hide the latest check from approval and malformed manifests still fail. No R2 or authentication mocks.
...
Production exits: 0 pass/warnings/empty selection, 1 failed/rejected/operational failure, 2 usage or missing/invalid configuration. Warnings (`not-verified`, `unsupported`, `delivered-spam`) do not prove delivery. Unsupported custom/multistep/payment/password/upload/draft/external flows, redirect-only confirmations, initialization delays, token mismatch, arbitrary site code and post-`wp_mail` transport changes remain safety limits, not reasons to bypass gates. Imports/no-form pages are lazy about forms/mail credentials; IMAP configuration is checked before any eligible opted-in click. Forms-only manifests have no visual evidence and are excluded from approval while sharing global last-ten retention.
